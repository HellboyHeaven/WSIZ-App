...
[Service]
public class AuthService(IMapper mapper, TokenService tokenService,
                         ...
                         IRefreshTokenRepository refreshTokenRepository)
{
    ...
    public async Task SignUpAsync(RegisterDto registerDto)
    {
        ...
        if (registerDto is StudentRegisterDto studentDto)
        {
            var student = _mapper.Map<Student>(studentDto);
            var course = await _courseRepository.GetByTitleAsync(studentDto.CourseName);
            student.Course = course;
            user = student;

        }
        else if (registerDto is TeacherRegisterDto teacherDto)
        {
            user = _mapper.Map<Teacher>(teacherDto); // Маппинг для Teacher
        }
        else if (registerDto is AdminRegisterDto adminDto)
        {
            user = _mapper.Map<Admin>(adminDto);
        }
        else
        {
